...
public class Zoo {
     ...
     * @return
     */
    boolean addZone(int maximumNoOfCages, String categoryOfAnimal,
        ...
        for (Zone.Category category : Zone.Category.values()) {
            if (category.name().equalsIgnoreCase(categoryOfAnimal)) {
                Zone newZone = new Zone(maximumNoOfCages, zoneId,
                        categoryOfAnimal, hasCanteen, hasPark);
     ...
     * @return
     */
    boolean addCage(String typeOfAnimal, int maximumCapacity, int zoneId) {
        ...
        if (isZoneAvailable) {
            for (AnimalEnum animal : AnimalEnum.values()) {
                if (animal.getType().equalsIgnoreCase(typeOfAnimal)
                        && animal.getCategory().equalsIgnoreCase(category)
                        && zone.getSparseCapacity() > 0) {
                    Cage cage = new Cage(typeOfAnimal, category,
                            maximumCapacity, cageId, zoneId);
                    cageId++;
                    isCageAdded = true;
     ...
     * @return
     */
    boolean addAnimal(String name, int age, double weight, String typeOfAnimal) {
        boolean isAnimalAdded = false;
        String categoryOfAnimal = null;
        if (name == null || name.length() == 0 || typeOfAnimal == null
                || typeOfAnimal.length() == 0) {
            throw new NullPointerException(
                    "Name Of Animal or Type Can't be Null or Zero");
        }
        ...
        if (isNameUnique(name)) {
            ...
            if (categoryOfAnimal != null) {
                for (int i = 0; i < cageList.size(); i++) {
                    if ((cageList.get(i).getTypeOfAnimal()
                            .equalsIgnoreCase(typeOfAnimal))
                            && (cageList.get(i).getSparseCapacity() > 0)) {
                        typeOfAnimal.toUpperCase();
                        isAnimalAdded = true;
                        switch (typeOfAnimal) {
                        case "LION":
                            animalList
                                    .add(new Lion(name, animalId, age, weight));
                            cageList.get(i).setSparseCapacity(
                                    cageList.get(i).getSparseCapacity() - 1);
                            animalId++;
                            break;

                        case "CROCODILE":
                            animalList.add(new Crocodile(name, animalId, age,
                                    weight));
                            cageList.get(i).setSparseCapacity(
                                    cageList.get(i).getSparseCapacity() - 1);
                            animalId++;
                        ...
                        case "PEACOCK":
                            animalList.add(new Peacock(name, animalId, age,
                                    weight));
                            cageList.get(i).setSparseCapacity(
                                    cageList.get(i).getSparseCapacity() - 1);
                            animalId++;
        ...
        for (int i = 0; i < animalList.size(); i++) {
            if (animalList.get(i).getName().equalsIgnoreCase(name)) {
                ...
                for (int j = 0; j < cageList.size(); j++) {
                    if (cageList.get(j).getTypeOfAnimal().equals(typeOfAnimal) && (cageList.get(j).getSparseCapacity() <
                            cageList.get(j).getMaximumCapacity()) ) {
                        cageList.get(j).setSparseCapacity(
                                cageList.get(j).getSparseCapacity() + 1);
                        break;
                    }
                    break;
                }
